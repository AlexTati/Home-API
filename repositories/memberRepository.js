...
const insertMember = function(member){
    return db.stmt("INSERT INTO `MEMBRE` (`Lastname`, `Firstname`, `Email`, `Phone`, `Username`, `Password`, `Home_street`, `Home_num`, `Home_box`, `Home_city_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
{
    return db.stmt("update MEMBRE set Lastname=?, Firstname=?, Email=?, Phone=?, Username=?, Password=?, Home_street=?, Home_num=?, Home_box=?, Home_city_id=? where Id=?",
        [member.Lastname, member.Firstname, member.Email, member.Phone, member.Username, member.Password, member.Home_street, member.Home_num, member.Home_box, member.Home_city_id]);
}
